...
int input_read_parameters(struct file_content * pfc,
 ...
 // ADF: parameters of VCDM which needs to be given
  class_read_double("log10b_VCDM",pba->log10b_VCDM);
  class_read_double("b2_bar_VCDM",pba->b2_bar_VCDM); // >0
  class_read_double("a1_VCDM",pba->a1_VCDM);  
  class_read_double("alpha_VCDM",pba->alpha_VCDM);  
...
int input_default_params(struct background *pba,
  ...
  pba->log10b_VCDM = 1.5;
  pba->a1_VCDM = 1.0/pow((1.0-pba->Omega0_g-pba->Omega0_b-pba->Omega0_cdm)/pba->Omega0_b,1.0/3.0);
  pba->b2_bar_VCDM = -1.0; // This should be positive, but if negative it chooses the non-agitated case
  pba->alpha_VCDM = 1.0;
