...
int background_functions(
  ...
  /* Lambda */
    /* Lambda */
    // ADF: here modified
  double alpha_VCDM = pba->alpha_VCDM; // ADF: value of alpha
  double a_star_VCDM = 0.; // ADF: value of a of transition
  /*if (pba->a1_VCDM < 0.) // ADF: in this case fix the parameter to a fixed value
    a_star_VCDM = 1.0/pow((1.0-pba->Omega0_g-pba->Omega0_b-pba->Omega0_cdm)/pba->Omega0_b,1.0/3.0);
  else
  a_star_VCDM = pba->a1_VCDM;*/
  // ADF: if given a negative value for a_star, then it defaults to a standard fixed value
  a_star_VCDM = (pba->a1_VCDM < 0.) ? 1.0/pow((1.0-pba->Omega0_g-pba->Omega0_b-pba->Omega0_cdm)/pba->Omega0_b,1.0/3.0): pba->a1_VCDM;
  pba->a_dagger_VCDM = a_star_VCDM;
    
  double b_VCDM = pow(10.0,pba->log10b_VCDM);
  double b2_bar_VCDM = pba->b2_bar_VCDM; // b2_VCDM = b2_min + b2_bar_VCDM
  double b2_VCDM = 0.;
  double rho0_today_VCDM = pba->Omega0_lambda * pow(pba->H0,2);
  if (b2_bar_VCDM > 0.)  { // b2_bar_VCDM needs to be positive, but for inital negative values it remove the agitated case
    // typical a of transition
    // double a_star_VCDM_typical = pow((0.1e1 - pba->Omega0_g - pba->Omega0_b - pba->Omega0_cdm) / pba->Omega0_b, -0.1e1 / 0.3e1);
    // Assuming rho0_VCDM \approx 0.7 H0^2 and a_star = a_star_VCDM_typical, we can deduce the minimum of b2 calculated as H^2(a=a_star_VCDM_typical) = 0 for rho0_VCDM \approx 0.7
    // double rho0_today_VCDM = pba->Omega0_lambda * pow(pba->H0,2);
    //double fraz = 0.7;
    //double rho0_VCDM_trial = 0.7*pow(pba->H0,2);
    double b2_VCDM_min = 0.;
      //(rho0_VCDM_trial * (alpha_VCDM - 0.1e1) * pow(a_star_VCDM, 0.4e1) - 0.2e1 * pow(pba->H0,2) * fraz * (pba->Omega0_b + pba->Omega0_cdm) * a_star_VCDM - 0.2e1 * pow(pba->H0,2) * fraz * pba->Omega0_g) / rho0_VCDM_trial * pow(a_star_VCDM, -0.4e1) / 0.2e1;
    //((0.7e1 * alpha_VCDM - 0.7e1) * pow(a_star_VCDM_typical, 0.4e1) - 0.20e2 * (pba->Omega0_b + pba->Omega0_cdm) * a_star_VCDM_typical - 0.20e2 * pba->Omega0_g) * pow(a_star_VCDM_typical, -0.4e1) / 0.14e2;
    b2_VCDM = b2_VCDM_min + b2_bar_VCDM;
  }
  pba->b2_VCDM = b2_VCDM;
  double rho0_VCDM = 0.2e1 * rho0_today_VCDM / (0.2e1 * b2_VCDM * exp(-b_VCDM * b_VCDM * pow(-0.1e1 + a_star_VCDM, 0.2e1) * pow(a_star_VCDM, -0.2e1)) + (-alpha_VCDM - 0.1e1) * tanh(b_VCDM * (-0.1e1 + a_star_VCDM) / a_star_VCDM) - alpha_VCDM + 0.1e1);
  pba->Lambda_s0_VCDM = 3.*rho0_VCDM; // derived parameter. Defining $rho_X(a=1) = \Lambda_{\rm s 0}/3$.
  //a_star_VCDM = 1.0/pow((1.0-pba->Omega0_g-pba->Omega0_b-pba->Omega0_cdm)/pba->Omega0_b,1.0/3.0);
  // ADF: next is rho_VCDM
  if (pba->has_lambda == _TRUE_) {
    // ADF: adding rhoX(a)
    pvecback[pba->index_bg_rho_lambda] = rho0_VCDM * (0.1e1 + alpha_VCDM) * (0.1e1 + tanh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM)) / 0.2e1 - alpha_VCDM * rho0_VCDM + rho0_VCDM * b2_VCDM * exp(-b_VCDM * b_VCDM * pow(a_star_VCDM, -0.2e1) * pow(a - a_star_VCDM, 0.2e1));
  ...
  double pX_VCDM = -pvecback[pba->index_bg_rho_lambda] -a/3.*rhoX_VCDM_a;

  p_tot += pX_VCDM;
  // ADF: Next is adding a*(d p_X / da), where pX is given above
  dp_dloga += -0.4e1 / 0.3e1 * (b2_VCDM * b_VCDM * (0.2e1 * pow(a_star_VCDM, 0.3e1) + a * (b_VCDM * b_VCDM - 0.5e1 / 0.2e1) * a_star_VCDM * a_star_VCDM - 0.2e1 * a * a * b_VCDM * b_VCDM * a_star_VCDM + pow(a, 0.3e1) * b_VCDM * b_VCDM) * exp(-b_VCDM * b_VCDM * pow(a_star_VCDM, -0.2e1) * pow(a - a_star_VCDM, 0.2e1)) - pow(0.1e1 / cosh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM), 0.2e1) * a_star_VCDM * a_star_VCDM * (0.1e1 + alpha_VCDM) * (tanh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM) * a * b_VCDM - 0.2e1 * a_star_VCDM) / 0.4e1) * b_VCDM * a * rho0_VCDM * pow(a_star_VCDM, -0.4e1);
...
int background_derivs(
  ...
  double b_VCDM = pow(10.0,pba->log10b_VCDM);
  double rho0_today_VCDM = pba->Omega0_lambda * pow(pba->H0,2);
  double rho0_VCDM = -0.2e1 * rho0_today_VCDM / (tanh(b_VCDM * (-0.1e1 + a_star_VCDM) / a_star_VCDM) * alpha_VCDM + tanh(b_VCDM * (-0.1e1 + a_star_VCDM) / a_star_VCDM) + alpha_VCDM - 0.1e1);

  double k2_pivot_VCDM = 0.1*0.1; // ADF: an arbitrarily chosen large value for k^2 in Mpc-1
  ...
  double rhoX_a = pow(0.1e1 / cosh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM), 0.2e1) * rho0_VCDM * (0.1e1 + alpha_VCDM) * b_VCDM / a_star_VCDM / 0.2e1;
  double rhoX_aa = -tanh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM) * pow(0.1e1 / cosh(b_VCDM * (a - a_star_VCDM) / a_star_VCDM), 0.2e1) * rho0_VCDM * (0.1e1 + alpha_VCDM) * b_VCDM * b_VCDM * pow(a_star_VCDM, -0.2e1);
